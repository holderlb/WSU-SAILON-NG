...
class CartPolePPNovel1(CartPoleBulletEnv):
    ...
    def __init__(self, difficulty, params: dict = None):
        ...
        # Load new cartpole file
        if self.difficulty == 'easy':
            self.cartpole_path = os.path.join(self.path, 'models', 'n1', 'easy.urdf')
        elif self.difficulty == 'medium':
            self.cartpole_path = os.path.join(self.path, 'models', 'n1', 'medium.urdf')
        elif self.difficulty == 'hard':
            self.cartpole_path = os.path.join(self.path, 'models', 'n1', 'hard.urdf')
        else:
    ...
    def reset_world(self):
        ...
        # Delete cartpole
        if self.cartpole == -10:
            self.cartpole = p.loadURDF(self.cartpole_path)
        else:
